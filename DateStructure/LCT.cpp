...
namespace LCT {
    ...
    inline void split(const int& u, const int& v) { evert(u), access(v), splay(v); }
    ...
    inline int Qry(const int& u, const int& v) { return split(u, v), datXor[v]; }
    inline void Mdy(const int& u, const int& x) { splay(u), val[u] = x, maintain(u); }
}
