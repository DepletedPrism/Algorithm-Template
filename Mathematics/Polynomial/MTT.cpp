...
namespace Poly {
  ...
  void FFT(Complex* f, const int& Lim, const int& type) {
    ...
    for (int Mid = 1; Mid < Lim; Mid <<= 1)
      for (int i = 0; i < Lim; i += Mid << 1)
        for (int j = 0; j < Mid; ++j) {
          Complex f0 = f[i+j], f1 = W[(LL) j * Lim / Mid] * f[i+j+Mid];
  ...
  void MTT(int* f, int n, int* g, int m, int* h) {
    ...
    for (int i = 0; i < Lim; ++i) {
      A[i] = (i < n)? Complex(f[i] & 32767, f[i] >> 15): Complex(0.0, 0.0);
      B[i] = (i < m)? Complex(g[i] & 32767, g[i] >> 15): Complex(0.0, 0.0);
    }

    init(Lim), FFT(A, Lim, 1), FFT(B, Lim, 1);
    for (int i = 0; i < Lim; ++i) {
      int j = (Lim - i) & (Lim - 1);
    ...
    for (int i = 0; i < Lim; ++i) {
      int da = LL(A[i].x / Lim + 0.5) % Mod, db = LL(A[i].y / Lim + 0.5) % Mod,
          dc = LL(B[i].x / Lim + 0.5) % Mod, dd = LL(B[i].y / Lim + 0.5) % Mod;
